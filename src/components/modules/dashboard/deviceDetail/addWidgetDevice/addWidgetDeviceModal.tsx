...
import WidgetDeviceDetailView from './deviceViews/widgetDeviceDetailView'
import {
...
export default function AddWidgetDeviceModal({
  ...
  const handleClose = () => {
    setSelectedWidget(DEVICE_WIDGET_OPTIONS[0] || null)
    setCurrentView('selection')
    onOpenChange(false)
  }
      ...
      <DialogContent className="max-w-5xl max-h-[80vh] flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-6 py-4">
          ...
          {currentView === 'detail' && selectedWidget && (
            <WidgetDeviceDetailView widget={selectedWidget} />
          )}
        </div>
